...
export const plantKbSchema: RxJsonSchema<PlantKbDocument> = {
  ...
  version: 5,
  description:
    "Expanded plant knowledge base (seasonality, pests, diseases, nutrients, sources) hydrated from plants-kb.json",
  primaryKey: "plant_id",
  type: "object",
  properties: {
...
export const inventorySchema: RxJsonSchema<InventoryDocument> = {
  ...
  properties: {
    id: { type: "string", maxLength: 100 },
    catalogId: { type: "string", maxLength: 100 },
...
export const plantedSchema: RxJsonSchema<PlantedDocument> = {
  ...
  properties: {
    id: { type: "string", maxLength: 100 },
    bedId: { type: "string", maxLength: 100 },
    catalogId: { type: "string", maxLength: 100 },
    gridX: { type: "number" },
    gridY: { type: "number" },
...
export const plantedMigrationStrategies = {
  1: function (oldDoc: MigrationDoc<CatalogDocument>) {
    // v0 → v1: Add observations array
    return { ...oldDoc, observations: [] };
  },
  2: function (oldDoc: MigrationDoc<CatalogDocument>) {
    // v1 → v2: Add systemDiagnosis field
    return { ...oldDoc, systemDiagnosis: "" };
  },
  3: function (oldDoc: MigrationDoc<PlantedDocument>) {
...
export const logbookSchema: RxJsonSchema<LogbookDocument> = {
  ...
  required: ["id", "type", "itemName", "date", "catalogId", "bedId"],
};
