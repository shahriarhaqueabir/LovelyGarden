...
const HarvestCard: React.FC<HarvestCardProps> = ({
  ...
  <div
    className={`glass-panel border p-4 rounded-2xl flex items-center justify-between group transition-all hover:translate-x-1 ${
      status === "success"
        ? "border-garden-500/20 hover:border-garden-500/40"
    ...
    <div className="flex items-center gap-4">
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center ${
          status === "success"
            ? "bg-garden-500/10 text-garden-500"
      ...
      <div>
        <div className="flex items-center gap-2">
          <h4 className="text-sm font-bold text-stone-200 uppercase tracking-tight">
            {entry.itemName}
          </h4>
          {status === "success" ? (
            <span className="text-[8px] font-black bg-garden-500/20 text-garden-400 px-1 py-0.5 rounded border border-garden-500/30 uppercase">
              Yield +1
            </span>
          ) : (
            <span className="text-[8px] font-black bg-red-500/20 text-red-400 px-1 py-0.5 rounded border border-red-500/30 uppercase">
              Lost
            </span>
          )}
        </div>
      ...
      <header className="p-8 border-b border-stone-800 bg-stone-900/30 backdrop-blur-md">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-stone-500 text-sm mt-1 uppercase tracking-widest font-bold">
              The Grimoire of Yields & Failures
            </p>
          </div>
          <div className="flex gap-4">
            <div className="bg-stone-900/50 border border-stone-800 px-4 py-2 rounded-2xl flex flex-col items-center min-w-[100px]">
              <span className="text-[9px] font-black text-stone-600 uppercase tracking-wider">
                Total Yields
              </span>
              <span className="text-xl font-black text-garden-400">
                {harvests.length}
              </span>
            </div>
            <div className="bg-stone-900/50 border border-stone-800 px-4 py-2 rounded-2xl flex flex-col items-center min-w-[100px]">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 border-b border-garden-500/20 pb-4">
              ...
              <h3 className="text-lg font-black text-stone-100 uppercase tracking-tighter">
                Golden Harvests
              </h3>
