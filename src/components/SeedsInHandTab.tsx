    ...
    const matchesFilter =
      ...
      (item.categories &&
        item.categories.includes(
          filter as import("../schema/knowledge-graph").PlantSpecies["categories"][0],
        ));
    ...
    <div className="flex flex-col h-full bg-[#0c0a09] text-stone-100 p-6 overflow-auto">
      <div className="mb-6">
        ...
        <p className="text-stone-400 text-sm">
          Your current collection of seeds ready for planting
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {sortedItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="p-4 bg-stone-800/20 rounded-xl border border-stone-700/30"
                >
          ...
          {sortedItems.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-stone-600 mt-2">
                Visit the seed store to acquire more seeds
              </p>
            </div>
          )}
        </div>
      ...
      <div className="mt-auto text-xs text-stone-600 p-4 bg-stone-900/20 rounded-xl border border-stone-800">
        <p>
          These are the seeds you currently have available to plant in your
          garden.
        </p>
      </div>
    </div>
