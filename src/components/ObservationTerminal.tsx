...
export const ObservationTerminal: React.FC<ObservationTerminalProps> = ({
  plant,
  catalog,
  onClose,
  onSave,
  ...
  const filteredObservations = patterns.filter(
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-[#0c0a09]">
            {view === "browse" ? (
              <div className="p-6 overflow-y-auto space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {filteredObservations.map((obs) => (
                    ...
                    >
                      <div>
                        <div className="text-sm font-bold text-stone-200 group-hover:text-garden-400 transition-colors">
                          {obs.label}
                        </div>
              ...
              <div className="flex-1 flex flex-col p-8 overflow-y-auto animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="mb-8">
                  <div className="text-[10px] font-black text-garden-400 uppercase tracking-widest mb-2 px-2 py-0.5 bg-garden-500/10 border border-garden-500/20 inline-block rounded">
                    Diagnostic Result
                  </div>
                  <h3 className="text-3xl font-black text-stone-100 leading-tight">
                ...
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="p-6 bg-stone-900/60 border border-stone-800 rounded-2xl relative overflow-hidden group">
                    ...
                    <div className="text-[10px] font-black text-stone-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 bg-garden-500 rounded-full" />{" "}
                      Reason / Diagnosis
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {currentObservation?.impact.hydration !== undefined && (
                      <div className="p-3 bg-stone-900/40 border border-stone-800 rounded-xl text-center">
                        <div className="text-[9px] font-black text-stone-500 uppercase mb-1">
                          Hydration
                        </div>
                        <div className="text-sm font-bold text-blue-400">
                          {currentObservation.impact.hydration}%
                        </div>
                      </div>
                    )}
                    {currentObservation?.impact.stress !== undefined && (
                      <div className="p-3 bg-stone-900/40 border border-stone-800 rounded-xl text-center">
                        ...
                        <div className="text-sm font-bold text-red-400">
                          +{currentObservation.impact.stress}%
                        </div>
                      </div>
                    )}
