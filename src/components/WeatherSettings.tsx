  ...
  useEffect(() => {
    if (storeLatitude !== null && storeLongitude !== null) {
      setLocationState((prev) => {
        ...
        if (
          storeLatitude !== prev.latitude ||
          storeLongitude !== prev.longitude
        ) {
        ...
        if (!newState.isCitySelected) {
          reverseGeocode(storeLatitude, storeLongitude)
            .then((foundCityName) => {
              if (
                foundCityName === "Unknown Location" ||
                foundCityName === "Location Lookup Failed"
              ) {
          ...
          {locationState.latitude !== null &&
            locationState.longitude !== null && (
              <div className="text-sm text-stone-400">
                ...
                {locationState.cityName !== "Select" &&
                  locationState.cityName !== "Unknown Location" &&
                  `. ${locationState.cityName}.`}
              </div>
            )}
        </div>
