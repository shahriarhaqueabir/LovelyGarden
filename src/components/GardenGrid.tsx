...
export const GridSlot: React.FC<GridSlotProps> = (props) => {
  ...
  const growthData = useMemo(() => {
    ...
    const daysElapsed = msToElapsedDays(planted, currentDay);

    return { completedStages: completed, daysElapsed, stage };
  ...
  const synergyScore = useMemo(() => {
    ...
    return calculateCompanionScore(
      item.catalogId,
      neighborIds,
      relationships || [],
    );
  ...
  const getBorderColor = () => {
    if (isOver)
      return "border-garden-400 border-2 shadow-[0_0_30px_rgba(34,197,94,0.4)] z-10";

    if (activeSeedCatalogId) {
      if (seedSynergy > 10)
        return "border-garden-400/60 shadow-[0_0_20px_rgba(34,197,94,0.3)]";
      if (seedSynergy < -10)
        return "border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.2)]";
    ...
    if (layer === "companions") {
      ...
      if (synergyScore > 10)
        return "border-garden-400 shadow-[0_0_20px_rgba(74,222,128,0.4)]";
      if (synergyScore < -10)
    ...
    if (layer === "hydration") {
      ...
      if (hydration < 50)
        return "border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.3)]";
      if (hydration < 75)
        return "border-blue-400/60 shadow-[0_0_15px_rgba(96,165,250,0.2)]";
    ...
    if (layer === "health") {
      ...
      if (isPestInfested)
        return "border-amber-400 shadow-[0_0_25px_rgba(251,191,36,0.4)] animate-pulse";
      if (stressLevel > 70)
    ...
    if (layer === "nutrients") {
      const avg = (n + p + k) / 3;
      if (avg < 30)
        return "border-purple-900 shadow-[0_0_15px_rgba(88,28,135,0.3)]";
      if (avg < 60)
        return "border-purple-500/50 shadow-[0_0_20px_rgba(168,85,247,0.3)]";
  ...
  const getOverlayLabel = () => {
    if (isDead)
      return (
        <span className="flex items-center gap-1">
          <Skull className="w-3 h-3" /> COMPOST
        </span>
      );
    if (isPestInfested)
      return (
        <span className="flex items-center gap-1">
          <Bug className="w-3 h-3" /> PEST ALERT
        </span>
      );
    ...
    if (layer === "hydration")
      return (
        <span className="flex items-center gap-1">
          <Droplets className="w-3 h-3" /> {Math.round(hydration)}% H2O
        </span>
      );
    ...
    if (layer === "companions") {
      ...
      return (
        <span className="flex items-center gap-1">
          <Heart
            className={`w-3 h-3 ${synergyScore > 0 ? "text-garden-400" : synergyScore < 0 ? "text-red-400" : "text-stone-500"}`}
          />
          {synergyScore > 0
            ? "SYNERGY"
            : synergyScore < 0
          ...
          <div className="w-full h-full p-3 flex flex-col items-center justify-between relative overflow-hidden shimmer-bg rounded-3xl">
            ...
            <div className="z-10 w-full flex justify-between items-start">
              ...
              <div className="flex gap-1">
                ...
                {hydration > 90 && stressLevel < 10 && (
                  <Sparkles className="w-4 h-4 text-garden-400" />
                )}
                {isPestInfested && (
                  <Bug className="w-4 h-4 text-amber-500 animate-bounce" />
                )}
              </div>
            </div>
                            ...
                            : layer === "health"
                              ...
                              : layer === "nutrients"
                                ...
                                : synergyScore > 0
                                  ...
                                  : synergyScore < 0
                                    ? "bg-red-900/30 border-red-500 text-red-400"
                                    : "bg-stone-900/30 border-stone-500 text-stone-400"
                    }`}
                  >
                    {getOverlayLabel()}
                  </span>
      ...
      {showActions && item && !isDead && (
        <div className="absolute top-2 right-2 flex gap-1 z-30">
          <button
            ...
            className="p-1.5 bg-stone-900/90 border border-stone-700/50 rounded-lg text-stone-400 hover:text-amber-400 transition-all"
          >
          ...
          <button
            ...
            className="p-1.5 bg-stone-900/90 border border-stone-700/50 rounded-lg text-stone-400 hover:text-red-400 transition-all"
          >
...
export const GardenField: React.FC<{
  ...
  const relationships = useMemo(() => {
    const rels: import("../schema/knowledge-graph").PlantRelationship[] = [];
    (catalog || []).forEach((c: CatalogDocument) => {
      (c.companions || []).forEach((targetId: string) => {
        rels.push({
          source_plant_id: c.id,
          target_plant_id: targetId,
          relationship: "beneficial",
          mechanism: [],
          confidence_score: 0.9,
          evidence_type: "horticultural_consensus" as const,
          sources: [],
        });
      });
