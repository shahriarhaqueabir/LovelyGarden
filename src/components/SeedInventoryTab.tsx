...
export const SeedInventoryTab: React.FC<SeedInventoryTabProps> = ({
  catalog,
}) => {
  ...
  const inventoryWithDetails = inventory
    .map((invItem) => {
      ...
      return {
        ...
        name:
          expandedItem?.common_name || catalogItem?.name || invItem.catalogId,
        scientificName:
          expandedItem?.scientific_name || catalogItem?.scientificName || "",
        ...
        water_requirements:
          expandedItem?.water_requirements ||
          catalogItem?.water_requirements ||
          "unknown",
        soil_type: expandedItem?.soil_type || catalogItem?.soil_type || [],
        seasonality: expandedItem?.seasonality || catalogItem?.seasonality,
        sowingSeason:
          expandedItem?.sowingSeason || catalogItem?.sowingSeason || [],
        sowingMethod:
          expandedItem?.sowingMethod || catalogItem?.sowingMethod || "Direct",
        companion_plants:
          expandedItem?.companion_plants || catalogItem?.companions || [],
        incompatible_plants:
          expandedItem?.incompatible_plants || catalogItem?.antagonists || [],
        common_pests:
          expandedItem?.common_pests || catalogItem?.common_pests || [],
        common_diseases:
          expandedItem?.common_diseases || catalogItem?.common_diseases || [],
        nutrient_preferences:
          expandedItem?.nutrient_preferences ||
          catalogItem?.nutrient_preferences ||
          [],
  ...
  const filteredItems = inventoryWithDetails.filter((item) => {
    ...
    const matchesFilter =
      filter === "all" ||
      (item.categories &&
        item.categories.some((cat) => cat.includes(filter))) ||
    ...
    <div className="flex flex-col h-full bg-[#0c0a09] text-stone-100 p-6 overflow-auto">
      <div className="mb-6">
        ...
        <p className="text-stone-400 text-sm">
          Your secure collection of acquired botanical assets
        </p>
      </div>
                  ...
                  >
                      ...
                      onClick={() => {
                        if (catalogItem) {
                          ...
                          setSelectedPlant({
                            ...catalogItem,
                            sowingSeason:
                              (expanded?.sowingSeason as Season[]) ||
                              catalogItem.sowingSeason ||
                              [],
                            sowingMethod:
                              (expanded?.sowingMethod as
                                | "Direct"
                                | "Transplant") ||
                            ...
                            stages:
                              expanded?.stages?.map((stage) => ({
                                ...
                                imageAssetId:
                                  stage.imageAssetId || "generic_image",
                              })) ||
                              catalogItem.stages ||
                              [],
                          });
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-bold text-stone-100 group-hover:text-garden-400 transition-colors">
                            {item.name}
                          </h3>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <div className="flex items-center gap-1 text-xs text-stone-400">
                          ...
                          <span className="capitalize">
                            {item.sunlight?.replace("_", " ") || "Unknown"}
                          </span>
                        </div>
                        <div className="flex items-center gap-1 text-xs text-stone-400">
                          {getWaterIcon(item.water_requirements)}
                          <span className="capitalize">
                            {item.water_requirements || "Unknown"}
                          </span>
                        </div>
                        <div className="flex items-center gap-1 text-xs text-stone-400">
                      ...
                      {item.soil_type && item.soil_type.length > 0 && (
                        <div className="mt-2 text-xs text-stone-500 truncate">
                          <span className="inline-flex items-center gap-1">
                            ...
                            <span>
                              Soil: {item.soil_type.slice(0, 2).join(", ")}
                            </span>
                            {item.soil_type.length > 2 && (
                              <span>+{item.soil_type.length - 2}</span>
                            )}
                          </span>
                        </div>
                      )}
      ...
      <div className="mt-auto text-xs text-stone-600 p-4 bg-stone-900/20 rounded-xl border border-stone-800">
        <p>
          Your seed vault contains all the seeds you've acquired for planting in
          your garden.
        </p>
      </div>
